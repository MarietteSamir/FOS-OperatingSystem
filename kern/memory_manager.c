...
void allocateMem(struct Env *e, uint32 virtual_address, uint32 size)
{
	...
	{
		int ret = pf_add_empty_env_page(e, (virtual_address + (PAGE_SIZE * i)), 0);
...
void __freeMem_with_buffering(struct Env *e, uint32 virtual_address, uint32 size)
{
	...
	for (int i = 0; i < NumOfNeededPages; i++)
	{
		pf_remove_env_page(e, (virtual_address + (PAGE_SIZE * i)));
	}
	...
	for (int i = 0; i < e->page_WS_max_size; i++)
	{
		...
		struct Frame_Info *ptr_frame_info = get_frame_info(e->env_page_directory, (void *)Address, &ptr_table);
		if (Address >= virtual_address && Address < (virtual_address + size))
		{
			if (ptr_frame_info != NULL)
			{
				unmap_frame(e->env_page_directory,(void*)Address);
				pt_clear_page_table_entry(e, Address);
				env_page_ws_clear_entry(e, i); // update the WS
			}
		}
	}
	...
	for (int i = 0; i < NumOfNeededPages; i++)
	{
		if (pt_get_page_permissions(e, (virtual_address + (PAGE_SIZE * i))) & PERM_BUFFERED)
		{
			uint32 *ptr_table = NULL;
			struct Frame_Info *ptr_frame_info = get_frame_info(e->env_page_directory, (void *)(virtual_address + (PAGE_SIZE * i)), &ptr_table);
			if (pt_get_page_permissions(e, (virtual_address + (PAGE_SIZE * i))) & PERM_MODIFIED)
			{
			...
			pt_clear_page_table_entry(e, (virtual_address + (PAGE_SIZE * i))); // Clear the page directory entry for the given virtual address
		}
	}
	...
	for (int i = 0; i < NumOfNeededPages; i++)
	{
		...
		get_page_table(e->env_page_directory, (void *)(virtual_address + (PAGE_SIZE * i)), &PageTablePointer);
		if (PageTablePointer != NULL)
		{
			bool EmptyPageTableFlag = 1;
			for (int j = 0; j < 1024 && EmptyPageTableFlag; j++)
			{
			...
			if (EmptyPageTableFlag)
			{
				kfree((void *)PageTablePointer);								 // Free the page table
				pd_clear_page_dir_entry(e, (virtual_address + (PAGE_SIZE * i))); // Clear the page directory entry for the given virtual address
			}
		}
	}
