...
void _main(void)
{
	...
	/*[1] CHECK INITIAL WORKING SET*/
	{
		...
		if (myEnv->page_last_WS_index != 0)
			panic("INITIAL PAGE WS last index checking failed! Review sizes of the two WS's..!!");
	}
	...
	if (sys_calculate_modified_frames() - initModBufCnt != 7)
		panic("Error in BUFFERING/RESTORING of modified frames of the slave ... WRONG number of buffered pages in MODIFIED frame list");
	...
	// cprintf("testing...\n");
	{
		...
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 1] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 2] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 3] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 4] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 5] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 6] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		if (arr[PAGE_SIZE * 1024 * 7] != -1)
		{
			sys_env_destroy(envIdSlave);
			panic("modified page not updated on page file OR not reclaimed correctly");
		}
		...
		{
			sys_env_destroy(envIdSlave);
			panic("Modified frames not removed from list (or isModified/modified bit is not updated) correctly when the modified list reaches MAX");
		}
	}
