...
void _main(void)
{
	...
	cprintf("STEP A: checking creation of shared object that is already exists... \n\n");
	{
		...
		x = smalloc("x", PAGE_SIZE, 1);
		if (x != NULL)
			panic("Trying to create an already exists object and corresponding error is not returned!!");
		if ((freeFrames - sys_calculate_free_frames()) != 0)
			panic("Wrong allocation: make sure that you don't allocate any memory if the shared object exists");
	}
	...
	cprintf("STEP B: checking the creation of shared object that exceeds the SHARED area limit... \n\n");
	{
		...
		y = smalloc("y", size, 1);
		if (y != NULL)
			panic("Trying to create a shared object that exceed the SHARED area limit and the corresponding error is not returned!!");
		if ((freeFrames - sys_calculate_free_frames()) != 0)
			panic("Wrong allocation: make sure that you don't allocate any memory if the shared object exceed the SHARED area limit");
	}
	...
	cprintf("STEP C: checking the creation of a number of shared objects that exceeds the MAX ALLOWED NUMBER of OBJECTS... \n\n");
	{
		...
		if (z != NULL)
			panic("Trying to create a shared object that exceed the number of ALLOWED OBJECTS and the corresponding error is not returned!!");
	}
