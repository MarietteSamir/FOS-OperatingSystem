...
void _main(void)
{
	...
	shMean = smalloc("mean", sizeof(int), 0);
	*shMean = mean;
	shVar = smalloc("var", sizeof(int), 0);
	*shVar = var;
	shMin = smalloc("min", sizeof(int), 0);
	*shMin = min;
	shMax = smalloc("max", sizeof(int), 0);
	*shMax = max;
	shMed = smalloc("med", sizeof(int), 0);
	*shMed = med;
...
int QSort(int *Elements, int NumOfElements, int startIndex, int finalIndex, int kIndex)
{
	...
	while (i <= j)
	{
		while (i <= finalIndex && Elements[startIndex] >= Elements[i])
			i++;
		while (j > startIndex && Elements[startIndex] <= Elements[j])
			j--;
