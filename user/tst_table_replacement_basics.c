...
void _main(void)
{
	...
	// cprintf("STEP B: checking the operations with page file... \n");
	{
		...
		if (arr[0] != -1)
			panic("make sure that you get the existing tables from page file");
		if (arr[PAGE_SIZE * 1024 - 1] != -1)
			panic("make sure that you get the existing tables from page file");
		if (arr[PAGE_SIZE * 1024 * 2 - 1] != -1)
			panic("make sure that you get the existing tables from page file");
		if (arr[PAGE_SIZE * 1024 * 3 - 1] != -1)
			panic("make sure that you get the existing tables from page file");
		if (arr[PAGE_SIZE * 1024 * 4 - 1] != -1)
			panic("make sure that you get the existing tables from page file");

		if (arr[PAGE_SIZE * 1024 * 5 - 1] != -1)
			panic("make sure that you get the existing tables from page file");
		if (arr[PAGE_SIZE * 1024 * 6 - 1] != -1)
			panic("make sure that you get the existing tables from page file");

		if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != 0)
			panic("tables not removed from page file correctly.. make sure you delete table from page file after reading it into memory");
