...
void _main(void)
{
	...
	if ((usedDiskPages - sys_pf_calculate_allocated_pages()) != 10 * (2 * Mega) / PAGE_SIZE)
	...
	if ((usedDiskPages - sys_pf_calculate_allocated_pages()) != 2 * Mega / PAGE_SIZE)
		panic("Wrong free: Extra or less pages are removed from PageFile");
	if ((sys_calculate_free_frames() - freeFrames) != 0)
		panic("Wrong free: Extra or less pages are removed from main memory");
	...
	if ((uint32)tempAddress != 0x81E01000)
		panic("Next Fit not working correctly");
	if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != (1 * Mega + 1016 * kilo) / PAGE_SIZE)
		panic("Wrong page file allocation: ");
	if ((freeFrames - sys_calculate_free_frames()) != 0)
		panic("Wrong allocation");
	...
	if ((uint32)tempAddress != 0x80400000)
		panic("Next Fit not working correctly");
	if ((sys_pf_calculate_allocated_pages() - usedDiskPages) != (3 * Mega + 512 * kilo) / PAGE_SIZE)
		panic("Wrong page file allocation: ");
	if ((freeFrames - sys_calculate_free_frames()) != 0)
		panic("Wrong allocation");
